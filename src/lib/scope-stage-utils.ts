...
/** Turn AI timeline text into a suggested due date (ISO yyyy-mm-dd). */
...
export function formatStageTimeline(stage: { timeline?: string | null; dueDate?: string | null }): string {
  if (stage.timeline?.trim()) return stage.timeline.trim();
  if (stage.dueDate) {
    try {
      return `Due ${new Date(stage.dueDate).toLocaleDateString("en-KE", { day: "numeric", month: "short", year: "numeric" })}`;
    } catch {
      return String(stage.dueDate);
    }
  }
  return "—";
}
