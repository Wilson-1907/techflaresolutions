import type { ScheduleStep } from "./schedule-progress";
...
export function workflowStagesToScheduleSteps(stages: WorkflowStage[]): ScheduleStep[] {
  return stages.map((s) => ({
    title: s.assignee ? `${s.title} · ${s.assignee}` : s.title,
